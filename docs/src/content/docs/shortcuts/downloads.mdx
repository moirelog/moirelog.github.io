---
title: 下载快捷指令
description: 下载并安装 Moire Config 和 Moire Sync 快捷指令
---

import { Steps, LinkButton, Card } from '@astrojs/starlight/components';

Moire 使用四个 iOS 快捷指令来实现同步功能。

Shortcuts 会不定期迭代，修复 Bug 和新增 Feature。推荐每隔一段时间重新下载并替换旧的 Shortcuts，以获得最佳体验。


## 下载地址


### 1. Moire Config

**作用**：存储你的 GitHub 配置信息（用户名、仓库名、 Token）。

<LinkButton href="/shortcuts/latest/config" variant="primary" icon="download" target="_blank">
  下载 Moire Config
</LinkButton>

### 2. 功能组件

这两个快捷指令负责具体的数据转换和上传功能。

<div class="flex gap-4 flex-wrap">
  <LinkButton href="/shortcuts/latest/upload-image" variant="secondary" icon="download" target="_blank">
    下载 Upload Image
  </LinkButton>
  <LinkButton href="/shortcuts/latest/clean-note" variant="secondary" icon="download" target="_blank">
    下载 Clean Note
  </LinkButton>
</div>

### 3. Moire Sync (主入口)

**作用**：将备忘录内容同步到 GitHub 的主程序。

<LinkButton href="/shortcuts/latest/sync" variant="secondary" icon="download" target="_blank">
  下载 Moire Sync
</LinkButton>


## 安装步骤

<Steps>

1. 下载并安装下方列出的 4 个快捷指令。

2. 完成！你应该在「快捷指令」App 中看到 4 个新快捷指令。

</Steps>



![](./moire-shortcuts.png)

## 版本信息

当前版本：**v1.0.1** (2026-02-15)

:::tip[检查更新]
快捷指令内部包含版本号。你可以打开快捷指令查看顶部的注释来确认版本。

为避免频繁的自动更新，例如在手机上修改非 Moire 文件夹的笔记，快捷指令会跳过修改时间超过 5 分钟以上的改动。
:::

## 下一步

安装完成后，我们需要配置 Moire Config。

import { LinkCard } from '@astrojs/starlight/components';

<LinkCard
  title="填写配置"
  description="设置 GitHub 用户名、仓库和 Token"
  href="/shortcuts/configuration/"
/>
