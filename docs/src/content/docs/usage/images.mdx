...
:::caution[图片删除说明]
目前 Moire 暂时没有处理对图片的删除操作。

为了防止 GitHub 仓库体积爆炸，请谨慎更换或删除笔记中的图片。如果你删除了笔记中的图片链接，之前上传到 `/src/memos/images/` 的图片文件仍会保留在仓库中。
:::
...
1. 快捷指令提取笔记中的第一张图片
2. 将图片转为 Base64 编码
3. 上传到 GitHub 仓库的 `/src/memos/images/` 目录
...
1. 将图片手动上传到仓库的 `/src/memos/images/`
2. 在笔记中使用相对路径引用
